  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 auto-rows-fr">
      {stats.map((stat) => {
        ...
        const card = (
          <div
            className="p-4 rounded-xl gradient-card shadow-card border-[4px] border-muted-foreground/50 transition-all duration-200 hover:border-muted-foreground/70 hover:scale-[1.03] hover:-translate-y-1 hover:shadow-elevated active:scale-[0.97] h-[180px] flex flex-col"
          >
            ...
            {'subtitle' in stat && stat.subtitle && (
              <p className={cn('text-[16px] mt-1', stat.color)}>{stat.subtitle}</p>
            )}
          </div>
        );
